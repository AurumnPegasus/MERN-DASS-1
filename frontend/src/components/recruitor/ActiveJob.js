...
const ActiveJob = () => {
        ...
        try {
            return (
                <div>
                    ...
                    <p>
                        <b>Applied On: </b>{app.apply}
                    </p>
                    <div className='row'>
                        <button className='hoverable blue accent3 waves-light btn waves-effect'
                            style={{margin: '10px'}}
                            onClick={() => changeStatus(app.title, app.myEmail, (app.status === 'shortlist' ? 'accept' : ((app.status === 'accept'? '': 'shortlist'))))}>
                            {(app.status === 'shortlist' ? 'accept' : ((app.status === 'accept'? '': 'shortlist')))}
                        </button>        
                        <button className='hoverable blue accent3 waves-light btn waves-effect'
                                onClick={() => changeStatus(app.title, store.user.email, (app.status === 'accept'? '' : 'reject'))}>
                            {(app.status === 'accept'? '' : 'reject')}
                        </button>
                    </div>
                    < hr />
                </div>
    ...
    const printSort = () => {
        ...
        if(sortApply) {
            setApps([...apps.sort((a, b) => (Date.parse(a.apply) < Date.parse(b.apply))? 1: -1 )])
        } else if (!sortApply){
            setApps([...apps.sort((a, b) => (Date.parse(a.apply) > Date.parse(b.apply))? 1: -1 )])
        }
    }
